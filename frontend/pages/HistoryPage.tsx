...
import { Shield, Clock, Globe, TrendingUp, Trash2, ChevronRight, ArrowLeft } from 'lucide-react';
...
export default function HistoryPage({ onBack, onLoadScan }: Props) {
  ...
  const planLabel = user?.plan === 'pro' ? 'Pro' : user?.plan === 'team' ? 'Team' : lang === 'fr' ? 'Gratuit' : 'Free';
  const planColor = user?.plan !== 'free' ? 'text-cyan-400 bg-cyan-500/10 border-cyan-500/30' : 'text-slate-400 bg-slate-800 border-slate-700';
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white font-bold font-mono text-lg">
                {lang === 'fr' ? 'Historique des scans' : 'Scan history'}
              </h1>
              <p className="text-slate-500 text-xs font-mono">{user?.email}</p>
            </div>
          </div>
          <span className={`text-xs font-mono px-2.5 py-1 rounded-full border ${planColor}`}>
            {planLabel}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <button onClick={onBack} className="mt-4 text-cyan-400 hover:text-cyan-300 text-sm font-mono transition">
