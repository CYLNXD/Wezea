"""
Scan Router — Endpoints de scan et génération PDF
==================================================
Endpoints extraits de main.py pour allèger le point d'entrée.

    GET  /client-id         → Initialise le cookie d'identification anonyme
    GET  /scan/limits       → Quota de scans de l'utilisateur courant
    POST /scan              → Lancer un audit de sécurité
    POST /report/request    → Demander le rapport PDF complet (Lead Gen)
    POST /generate-pdf      → Générer le rapport PDF expert
"""
...
from app.services import brevo_service, report_service
...
logger = logging.getLogger("cyberhealth.scan")
_lead_logger = logging.getLogger("cyberhealth.lead")

router = APIRouter(tags=["Scan"])

_DEBUG = os.getenv("DEBUG", "false").lower() == "true"
GLOBAL_SCAN_TIMEOUT_SEC = int(os.getenv("GLOBAL_SCAN_TIMEOUT_SEC", "60"))


# ─────────────────────────────────────────────────────────────────────────────
# SSRF protection — importée depuis main pour éviter la duplication
# ─────────────────────────────────────────────────────────────────────────────

def _resolve_and_check_ssrf(domain: str) -> None:
    """Proxy vers la fonction dans main.py (importée dynamiquement pour éviter les cycles)."""
    from app.main import _resolve_and_check_ssrf as _check
    _check(domain)


# ─────────────────────────────────────────────────────────────────────────────
# Constantes de validation (partagées avec main.py)
# ─────────────────────────────────────────────────────────────────────────────
...
async def get_scan_limits(request: Request) -> dict:
    ...
    - Pro/Team : illimité
    """
    from app.services.rate_limit_service import ANON_SCAN_LIMIT as _ANON_LIMIT
    ...
    try:
        ...
        if current_user:
            ...
            return {
                ...
                "limit":     _ANON_LIMIT,
                "used":      used,
                "remaining": max(0, _ANON_LIMIT - used),
                "day_key":   day,
            }
    finally:
        db.close()
