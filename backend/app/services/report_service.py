...
def _build_context(
    ...
    return {
        ...
        **_checks_context(data, findings, lang),
        ...
        # Conformité NIS2 / RGPD — critères + articles via le mapper unifié
        "compliance":        _build_compliance_context(findings, lang),
    }


# ── Conformité NIS2 / RGPD ────────────────────────────────────────────────────
# Source unique : compliance_mapper.py — plus de duplication ici.
...
def _build_compliance_context(findings: list[dict], lang: str = "fr") -> dict:
    ...
    Délègue au ComplianceMapper (source unique de vérité).
    ...
    from app.compliance_mapper import (
        ComplianceMapper, STATUS_LABEL, STATUS_COLOR,
    )

    mapper = ComplianceMapper()
    compliance = mapper.analyze(findings)
    criteria_results = compliance.criteria

    labels = STATUS_LABEL.get(lang, STATUS_LABEL["fr"])
    label_key = "label_fr" if lang != "en" else "label_en"
    desc_key  = "desc_fr"  if lang != "en" else "desc_en"
    article_key = "article_fr" if lang != "en" else "article_en"
    ...
    for cr in criteria_results:
        ...
        results.append({
            "id":           cr_dict["id"],
            "label":        cr_dict[label_key],
            "regulations":  cr_dict["regulations"],
            "article":      cr_dict[article_key],
            "desc":         cr_dict[desc_key],
            "status":       cr_dict["status"],
            "status_label": labels.get(cr_dict["status"], cr_dict["status"]),
            "status_color": STATUS_COLOR.get(cr_dict["status"], "#94a3b8"),
        })
    ...
    disclaimer = compliance.disclaimer_fr if lang != "en" else compliance.disclaimer_en
    ...
    return {
        ...
        "overall_color":   STATUS_COLOR.get(overall, "#94a3b8"),
